...
describe AcmeV1::Client::SelfSignCertificate do
  it 'generate a self sign certificate' do
    self_sign_certificate = AcmeV1::Client::SelfSignCertificate.new(subject_alt_names: ['test.example.org'])
  ...
  it 'generate a self sign certificate from a provided private key' do
    ...
    self_sign_certificate = AcmeV1::Client::SelfSignCertificate.new(private_key: private_key, subject_alt_names: ['test.example.org'])
    expect(self_sign_certificate.certificate).to be_a(OpenSSL::X509::Certificate)
