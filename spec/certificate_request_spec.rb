...
describe AcmeV1::Client::CertificateRequest do
  let(:test_key) { generate_private_key }

  it 'reads the common name from the subject' do
    request = AcmeV1::Client::CertificateRequest.new(private_key: test_key, subject: { common_name: 'example.org' })

    expect(request.common_name).to eq('example.org')

    request = AcmeV1::Client::CertificateRequest.new(private_key: test_key, subject: { 'CN' => 'example.org' })
  ...
  it "doesn't modify the given subject" do
    ...
    AcmeV1::Client::CertificateRequest.new(private_key: test_key, subject: subject)

    expect(subject).to eq(original)
  end

  it 'normalizes the subject to OpenSSL short names' do
    subject = AcmeV1::Client::CertificateRequest::SUBJECT_KEYS.each_with_object({}) {|(key, _), hash|
      hash[key] = 'example'
    }
    request = AcmeV1::Client::CertificateRequest.new(private_key: test_key, subject: subject)

    subject = AcmeV1::Client::CertificateRequest::SUBJECT_KEYS.each_with_object({}) {|(_, short_name), hash|
  ...
  it 'sets the subject common name from the parameter' do
    request = AcmeV1::Client::CertificateRequest.new(common_name: 'example.org', private_key: test_key)
  ...
  it 'picks a single domain as the common name' do
    request = AcmeV1::Client::CertificateRequest.new(names: %w(example.org), private_key: test_key)
  ...
  it 'picks the common name from the names' do
    request = AcmeV1::Client::CertificateRequest.new(names: %w(example.org www.example.org), private_key: test_key)
  ...
  it 'expects a domain' do
    expect {
      AcmeV1::Client::CertificateRequest.new(private_key: test_key)
    }.to raise_error(ArgumentError, /No common name/)
  end

  it 'disallows arbitrary subject keys' do
    expect {
      AcmeV1::Client::CertificateRequest.new(
  ...
  it 'adds other valid attributes to the subject' do
    subject_keys = AcmeV1::Client::CertificateRequest::SUBJECT_KEYS
  ...
  it 'supports ECDSA keys' do
    ...
    ec_key.generate_key
    request = AcmeV1::Client::CertificateRequest.new(common_name: 'example.org',
                                                   private_key: ec_key)
